...
- п'ять ролей із різними правами, які перевіряються на сервері;
...
| Менеджер | `manager@courier.ua` | Перегляд усього, але без права щось змінити |
...
| `GET` | `/api/v1/users/customers` | ADMIN, DISPATCHER, MANAGER |
| `GET` | `/api/v1/users/{id}` | ADMIN, MANAGER або сам користувач |
...
| Метод | Шлях | Доступ |
| --- | --- | --- |
| `GET` | `/api/v1/orders` | ADMIN, DISPATCHER, MANAGER - усі; CUSTOMER - власні |
...
| Дія | ADMIN | DISPATCHER | MANAGER | CUSTOMER | COURIER |
| --- | :---: | :---: | :---: | :---: | :---: |
